...
S1 = ["30fb40d4", "9fa0ff0b", "6beccd2f", "3f258c7a", "1e213f2f", "9c004dd3", "6003e540", "cf9fc949",
...
S2 = ["1f201094", "ef0ba75b", "69e3cf7e", "393f4380", "fe61cf7a", "eec5207a", "55889c94", "72fc0651",
...
S3 = ["8defc240", "25fa5d9f", "eb903dbf", "e810c907", "47607fff", "369fe44b", "8c1fc644", "aececa90",
...
S4 = ["9db30420", "1fb6e9de", "a7be7bef", "d273a298", "4a4f7bdb", "64ad8c57", "85510443", "fa020ed1",
...
def f1(_D, _Kr, _Km):
    ...
    # print('Km: ', Km)
    # print('D: ', _D)
    I = (bin(((int(_Km) + int(_D)) % (2 ** 32)) << int(_Kr) % (2 ** 32)))[2:]
    # print('I: ', I)
    # if len(I) < 32:
    #     s = 32 - len(I)
    #     for q in range(s):
    ...
    Id = int(I[24:32], 2)
    # print(Id)
    O = (((int(S1[Ia], 16) | int(S2[Ib], 16)) - int(S3[Ic], 16)) + int(S4[Id], 16)) % (2 ** 32)
    return O


def f2(_D, _Kr, _Km):
    I = bin(int(_Km) | int(_D) << int(_Kr))[2:]
    Ia = int(I[0:8], 2)
    Ib = int(I[8:16], 2)
    Ic = int(I[16:24], 2)
    Id = int(I[24:32], 2)
    O = (((int(S1[Ia], 16) - int(S2[Ib], 16)) + int(S3[Ic], 16)) % (2 ** 32)) | int(S4[Id], 16)
    return O


def f3(_D, _Kr, _Km):
    I = bin((int(_Km - _D) % (2 ** 32)) << int(_Kr))[2:]
    ln = len(I)
    Ia = int(I[0:8], 2)
    Ib = int(I[8:16], 2)
    Ic = int(I[16:24], 2)
    Id = int(I[24:32], 2)
    O = ((((int(S1[Ia], 16) + int(S2[Ib], 16)) % (2 ** 32)) | int(S3[Ic], 16)) - int(S4[Id], 16)) % (2 ** 32)
    return O


def Q(ABCD, i, Kr, Km):
    ABCD = bin(int(ABCD, 16))[2:]
    # print(ABCD)
    if ABCD == "0":
        A = 0
        B = 0
        ...
        C = int(ABCD[64:96], 2)
        D = int(ABCD[96:128], 2)
    C = C | f1(D, Kr[0][i], Km[0][i])
    B = B | f2(C, Kr[1][i], Km[1][i])
    A = A | f3(B, Kr[2][i], Km[2][i])
    D = D | f1(A, Kr[3][i], Km[3][i])
    return str(A) + str(B) + str(C) + str(D)


def QBAR(ABCD, i, Kr, Km):
    ABCD = bin(int(ABCD, 16))[2:]
    if ABCD == "0":
        A = 0
        B = 0
...
def W(i, KAPPA):
    # print(bin(int(KAPPA, 16))[2:])
    KAPPA = bin(int(KAPPA, 16))[2:]
    A = int(KAPPA[0:32], 2)
    print('A:\n', bin(A))
    B = int(KAPPA[32:64], 2)
    C = int(KAPPA[64:96], 2)
    D = int(KAPPA[96:128], 2)
    E = int(KAPPA[128:160], 2)
    F = int(KAPPA[160:192], 2)
    G = int(KAPPA[192:224], 2)
    H = int(KAPPA[224:256], 2)
    G = G | f1(H, Tr[0][i], Tm[0][i])
    F = F | f2(G, Tr[1][i], Tm[1][i])
    E = E | f3(F, Tr[2][i], Tm[2][i])
    D = D | f1(E, Tr[3][i], Tm[3][i])
    C = C | f2(D, Tr[4][i], Tm[4][i])
    B = B | f3(C, Tr[5][i], Tm[5][i])
    A = A | f1(B, Tr[6][i], Tm[6][i])
    print('f1(B, Tr[6][i], Tm[6][i]): \n', bin(f1(B, Tr[6][i], Tm[6][i])))
    print('newA:\n', bin(A))
    H = H | f2(A, Tr[7][i], Tm[7][i])
...
def _5LSB(x):
    ...
    ln = len(x)
    print('Binary x: ', x)
    return int((x[ln - 5:ln]), 2)


def CAST_256(_BETA, Kr, Km):
    for k in range(6):
        _BETA = Q(_BETA, k, Kr, Km)
    for k in range(6, 12):
        _BETA = QBAR(_BETA, k, Kr, Km)
    return _BETA
...
if __name__ == '__main__':
    ...
    Tm = np.ndarray((8, 24))
    Tr = np.ndarray((8, 24))
    Kr = np.ndarray((4, 12), dtype=int)
    Km = np.ndarray((4, 12), dtype=int)
    KrNEW = np.ndarray((4, 12), dtype=int)
    KmNEW = np.ndarray((4, 12), dtype=int)
    # print(T)
    s = len(symbols)
    open_text = ''
    ...
    #     for j in range(16):
    #         open_text += symbols[rnd.randint(0, s - 1)]


    # print(sys.getsizeof(open_text))

    # CAST_256(open_text)
    # print(bin(int(S1[0], 16)), len(bin(int(S1[0], 16))))
    ...
    # PRIMARY KEY
    KAPPA = "2342bb9efa38542cbed0ac83940ac2988d7c47ce264908461cc1b5137ae6b604"
    ...
    for i in range(12):
        ...
        print('\n ================== ROUND No', i + 1, ' =================== \n')
        Kr[0][i] = _5LSB(KAPPA[0:4])     # 5LSB(A)
        Kr[1][i] = _5LSB(KAPPA[8:12])    # 5LSB(C)
        Kr[2][i] = _5LSB(KAPPA[16:20])   # 5LSB(E)
        Kr[3][i] = _5LSB(KAPPA[24:28])   # 5LSB(G)
        print('Kr[0]: ', Kr[0][i])
        print('Kr[1]: ', Kr[1][i])
        print('Kr[2]: ', Kr[2][i])
        print('Kr[3]: ', Kr[3][i])

        Km[0][i] = KAPPA[28:32]          # H
        Km[1][i] = KAPPA[20:24]          # F
        Km[2][i] = KAPPA[12:16]          # D
        Km[3][i] = KAPPA[4:8]            # B
        print('Km[0]: ', Km[0][i])
        print('Km[1]: ', Km[1][i])
        print('Km[2]: ', Km[2][i])
